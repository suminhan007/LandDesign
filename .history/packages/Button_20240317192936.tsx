...
const Button: React.FC<ButtonProps> = ({
  ...
  return (
    <>
      {!href ?
        <StyledButton
          className={`land-button ${type} ${status} ${iconOnly ? 'iconOnly' : ''} ${disabled ? 'disabled' : ''} ${pop ? 'hover-pop' : ''} ${className}`}
          style={style}
          width={width}
          height={height}
          justify={justify}
...
const StyledButton = styled.div<{
  ...
  height?: string;
    ...
    gap: var(--size-8);
    padding: ${props => (props.width && props.width !== '100%') ? '' : 'var(--padding-medium)'};
