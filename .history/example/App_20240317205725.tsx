...
function App() {
  const [activedId, setActivedId] = useState<number>(101);
  ...
  return (
    <div>
      <StyledTopNav className='flex px-24'>
        <Title title='Land Design' style={{ color: 'var(--color-primary-6)' }} />
      </StyledTopNav>
      <div className='flex'>
              ...
              />
          {
            Nav_Data.map(navItem =>
              <div className='flex column gap-8'>
                <Title title={`-- ${navItem.title}`} type='p' className='my-8' style={{ color: 'var(--color-primary-4)' }} />
                {
        ...
        <StyledRightContent className='flex column p-24 overflow-auto'>
          ...
          {
            activedId != 0 && Nav_Data[activedPage-1].data.filter(itm => itm.id === activedId).map(item =>
              <>
